...
Write-Host "Starting the service -> ${PcAddress}:$Port at $Rate Hz (frames $FrameRate Hz, autostart $autostartValue) ..."
& $Adb shell am start-foreground-service -n "$package/.TrackingService" -a "$package.START" `
    --es host $PcAddress --ei port $Port --ef rate $Rate --ef framerate $FrameRate --ez autostart $autostartValue | Out-Null
