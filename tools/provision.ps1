...
param(
    ...
    [switch]$Always,
    # Also forward the gaze pose. Off by default: leaving the eye tracker's gaze to Virtual
    # Desktop makes it compatible with VD Eye Tracking Foveated Encoding (VD's own VRCFT
    # module supplies the gaze then)
    [switch]$Gaze,
    [switch]$NoEye,
    [switch]$NoFace,
    [switch]$Autostart,
...
$gazeValue = if ($Gaze) { "true" } else { "false" }
...
Write-Host "Starting the service -> ${PcAddress}:$Port at $Rate Hz (frames $FrameRate Hz, gaze $gazeValue, eye $eyeValue, face $faceValue, $gateText, autostart $autostartValue) ..."
& $Adb logcat -c
if ($gateList) {
    & $Adb shell am start-foreground-service -n "$package/.TrackingService" -a "$package.START" `
        ...
        --ez gaze $gazeValue --ez eye $eyeValue --ez face $faceValue `
